...
export const getRelatedCells = (row: number, col: number): {row: number, col: number}[] => 
  cellCoords.filter(({row: y, col: x}) => 
    (y !== row || x !== col) && 
    (y === row || x === col || (Math.floor(y / 3) === Math.floor(row / 3) && Math.floor(x / 3) === Math.floor(col / 3))));
...
export const getConflictCells = (grid: Grid): {row: number, col: number}[] => 
  toUniqueCells(cellCoords
    .flatMap(({row, col}) => getRelatedCells(row, col)
    .filter(({row: y, col: x}) => grid[row][col] !== 0 && grid[row][col] === grid[y][x])));
...
export const getAutoNotes = (grid: Grid): Notes => 
  cellCoords.filter(({row, col}) => grid[row][col] === 0)
    .map(({row, col}): [string, Set<number>] => [`${row}-${col}`, new Set([1, 2, 3, 4, 5, 6, 7, 8, 9]
    .filter((num) => isValidCell(grid, row, col, num)))]).toObject();
