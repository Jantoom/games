...
export const HintsButton: React.FC<HintsButtonProps> = ({
  ...
  const showMismatches = () => showErrorAnimation(toCellKeys(getConflictCells(grid)));
  const validateGrid = () => showErrorAnimation(toCellKeys(getMismatchCells(grid, solvedGrid)));
  const addAutoNotes = () => setState(prevState => ({ notes: {...prevState.notes, ...getAutoNotes(prevState.grid)}}));
  ...
  return (
    <Dialog onOpenChange={isOpen => isOpen ? null : close() }>
      ...
      <DialogContent className="border-border [&>button:last-child]:hidden">
        ...
        <DialogClose asChild>
          <Button onClick={getHint} variant="outline" className="w-full border border-border hover:bg-secondary">
            Get Hint
          </Button>
        </DialogClose>
        <DialogClose asChild>
          <Button onClick={showMismatches} variant="outline" className="w-full border border-border hover:bg-secondary">
            Show Mismatches
          </Button>
        </DialogClose>
        <DialogClose asChild>
          <Button onClick={validateGrid} variant="outline" className="w-full border border-border hover:bg-secondary">
            Validate Grid
          </Button>
        </DialogClose>
        <DialogClose asChild>
          <Button onClick={addAutoNotes} variant="outline" className="w-full border border-border hover:bg-secondary">
            Add Auto Notes
          </Button>
        </DialogClose>
        <DialogFooter>
